...
@SpringBootTest
@ActiveProfiles("test")
class PlanetFilterServiceImplIntegrationTest {

    @Autowired
    private PlanetFilterService filterService;
    ...
    void whenFilterNameSubstring_thenReturnsMatches() {
        var result = filterService.filter(DATA, new FilterDto("der", null, null));
        assertThat(result).extracting(PlanetDto::name).containsExactly("Alderaan");
    }
    ...
    void whenFilterFromExactTimestamp_thenIncludesExactAndAfter() {
        ...
        assertThat(result).extracting(PlanetDto::id).containsExactly(2L, 3L);
    }
    ...
    void whenFilterFromAfterTimestamp_thenExcludesPrevious() {
        ...
        var result = filterService.filter(DATA, new FilterDto(null, from, null));
        assertThat(result).extracting(PlanetDto::id).containsExactly(3L);
    }
    ...
    void whenFilterToExactTimestamp_thenIncludesBeforeAndExact() {
        ...
        assertThat(result).extracting(PlanetDto::id).containsExactly(1L, 2L);
    }
